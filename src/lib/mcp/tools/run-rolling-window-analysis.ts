...
import { MAX_WINDOW_YEARS, MIN_WINDOW_YEARS } from "@/lib/mcp/limits";
import {
  indexSchema,
  isoDate,
  presetSchema,
  ...
  server.registerTool(
    "run_rolling_window_analysis",
    {
      ...
      description:
        ...
        "drawdown, and win rate. Use `smaEnabled` for SMA timing. NOT investment advice.",
      inputSchema: {
    ...
    async (args) => {
      try {
        ...
        return toolSuccess(
          summary,
          withDisclaimer({ windowLengthYears: windowLength, startDate, endDate, analysis: stats }),
        );
      } catch (error) {
