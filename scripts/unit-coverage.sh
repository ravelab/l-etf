#!/bin/sh
# Unit-suite coverage gate. 80% on lines / functions / branches is a floor — same
# bar carpool-max keeps. UI/browser suites are not counted (they hit a built bundle).
set -eu
...
then
  ...
  printf '✗ coverage run failed (need ≥80%% lines/functions, ≥70%% branches)\n\n' >&2
  # Prefer the coverage table + failure reason over the full TAP dump.
